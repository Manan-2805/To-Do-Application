  ...
  if (total === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "240px" }}>
        ...
        <span style={{ marginTop: "1rem", color: "var(--text-secondary)", fontSize: "0.875rem" }}>No tasks created yet</span>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: "2rem" }}>
      <div style={{ position: "relative", width: "160px", height: "160px" }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: "1.75rem", fontWeight: 700, fontFamily: "var(--font-display)" }}>{total}</span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        {data.map((item, idx) => (
          <div key={idx} style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.875rem" }}>
            ...
            <span style={{ color: "var(--text-secondary)", fontWeight: 550, width: "90px" }}>{item.label}</span>
            <span style={{ fontWeight: 700 }}>{item.value}</span>
            <span style={{ color: "var(--text-secondary)", fontSize: "0.75rem" }}>({Math.round((item.value / total) * 100)}%)</span>
          </div>
        ))}
      </div>
    </div>
