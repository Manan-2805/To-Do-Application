...
class S3StorageProvider(StorageProvider):
    ...
    async def save_file(self, file_name: str, contents: bytes) -> str:
        ...
        try:
            ...
            logger.debug(
                "Uploaded file to S3. key=%s size=%d", file_name, len(contents)
            )
            return file_name
        except Exception as exc:
            logger.error("S3 upload failed. key=%s error=%s", file_name, exc)
            raise StorageException(
                f"Failed to upload file '{file_name}' to S3: {exc}"
            ) from exc
    ...
    async def get_file(self, file_path: str) -> bytes:
        ...
        try:
            ...
            logger.debug(
                "Downloaded file from S3. key=%s size=%d", file_path, len(data)
            )
            return data
        except Exception as exc:
            logger.error("S3 download failed. key=%s error=%s", file_path, exc)
            raise StorageException(
                f"Failed to download file '{file_path}' from S3: {exc}"
            ) from exc
    ...
    async def delete_file(self, file_path: str) -> None:
        ...
        try:
            ...
            raise StorageException(
                f"Failed to delete file '{file_path}' from S3: {exc}"
            ) from exc
    ...
    async def check_health(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "S3 health check failed. bucket=%s error=%s", self._bucket, exc
            )
            return False
